...
import Landing3D from "../components/ui/Landing3D";

import { motion } from "framer-motion";

const Hero = () => {
  const [fontSize, setFontSize] = useState("172px");
      ...
      <div className="px-6  h-screen w-full min-w-screen min-h-screen md:h-screen md:max-h-screen  lg:grid lg:grid-cols-[60%,40%] grid-rows-1 gap-4 grid grid-cols-1 ">
        <div className=" text-[#97b8ff] h-full flex flex-col justify-center">
          <TypewriterEffect
            ...
            startDelay={1000}
            deleteSpeed={50}
            onTypingComplete={() => setShowOtherText(true)}
          />
          <div
            className={`  transition-opacity duration-500 opacity-0 ${
              showOtherText
                ? "opacity-100 transition-all duration-500 ease-linear "
                : ""
            }`}
          >
            <h3
              className={`md:text-3xl text-2xl py-4 font-spaceGrotesk-semibold mb-4 `}
            >
            ...
            <button
              className={`my-6 group cursor-pointer relative cursor-default w-[120px] h-[50px] bg-[linear-gradient(144deg,_#af40ff,_#5b42f3_50%,_#00ddeb)] text-white whitespace-nowrap flex flex-wrap rounded-lg overflow-hidden transition-opacity duration-1000 opacity-0  ${
                showOtherText
                  ? " opacity-100 transition-all duration-1500 ease-linear"
                  : "opacity-0 "
              }`}
            >
              <span className=" absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 pointer-events-none text-lg font-spaceGrotesk-bold">
                Learn More
              </span>
